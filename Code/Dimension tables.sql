create or replace table cricket.consumption.date_dim (
    ...
    dayofmonth int,
    dayofyear int,
    dayname varchar(3), -- to store day names (e.g., "Mon")
...
-- Inserting Data into these facts and Dimension tables now
...
(
select td.team_id,pc.players 
from cricket.clean.player_details_clean pc 
join cricket.consumption.team_dim td on pc.team = td.team_name
group by td.team_id, pc.players );
...
insert into cricket.consumption.referee_dim (referee_name,referee_type,match_number_type)
(
select 
case 
when o.path = 'umpires' then v.value::string
else o.value[0]::string
end as referee_name,
o.path::string as referee_type,
info:match_type_number::number as match_type_number
from
cricket.raw.match_raw_tbl ,
lateral flatten (input => info:officials) o,
lateral flatten (input => o.value) v
);
...
(
select venue,city from cricket.clean.match_detail_clean group by 1,2
) ;

select * from cricket.consumption.venue_dim where city = 'Hyderabad';


-- 5. Match Type Dim table
...
-- Creating a table to contain all the date range from Min(EVENT_DATE)  to (Max(EVENT_DATE)

select  max(EVENT_DATE) ,min(EVENT_DATE) from cricket.clean.match_detail_clean;


CREATE or replace transient TABLE cricket.consumption.date_rnage01 (Date DATE);

truncate table cricket.consumption.date_rnage01;
...
INSERT INTO cricket.consumption.date_dim (Date_ID, Full_Dt, Day, Month, Year, Quarter, DayOfWeek, DayOfMonth, DayOfYear, DayName, IsWeekend)
SELECT
